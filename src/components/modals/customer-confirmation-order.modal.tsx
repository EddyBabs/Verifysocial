...
const CustomerOrderConfirmationHelper = ({
  orderId,
  disableCancel,
  showOrderConfirmationModal,
  setShowCustomerOrderConfirmationModal,
}: {
  disableCancel: boolean;
  orderId: string;
  showOrderConfirmationModal: boolean;
  setShowCustomerOrderConfirmationModal: Dispatch<SetStateAction<boolean>>;
}) => {
  ...
  const onSubmit = async (values: any) => {
    ...
    if (response.success) {
      ...
      router.refresh();
      setShowCustomerOrderConfirmationModal(false);
    } else {
  ...
  const handleOpenChange = () => {
    ...
    setShowCustomerOrderConfirmationModal((prev) => !prev);
  };
...
export function useCustomerOrderConfirmationModal(disableCancel = false) {
  const [
    showCustomerOrderConfirmationModal,
    setShowCustomerOrderConfirmationModal,
  ] = useState(false);
  ...
  const AddOrderConfirmationModal = useCallback(() => {
    return (
      <CustomerOrderConfirmationHelper
        orderId={orderId}
        disableCancel={disableCancel}
        showOrderConfirmationModal={showCustomerOrderConfirmationModal}
        setShowCustomerOrderConfirmationModal={
          setShowCustomerOrderConfirmationModal
        }
      />
    );
  }, [disableCancel, orderId, showCustomerOrderConfirmationModal]);
  ...
  return useMemo(
    () => ({
      ...
      openModal: (orderId: string) => {
        setOrderId(orderId);
        setShowCustomerOrderConfirmationModal(true);
      },
    }),
    [AddOrderConfirmationModal, showCustomerOrderConfirmationModal]
  );
}
